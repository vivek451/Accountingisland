...
class DashboardView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        sales, expenses, profit_loss = calculate_profit_loss(self.request.user)
        balance_sheet_data = balance_sheet_view(self.request.user)
        cashflow_data = cashflow(self.request)
        show_back_button = False
        profit_loss_info = {'sales': sales, 'expenses': expenses,
                            'profit_loss': profit_loss, 'show_back_button': show_back_button}

        context.update(profit_loss_info)
        context.update(balance_sheet_data)
        context.update(cashflow_data)
...
class TransactionListView(LoginRequiredMixin, ListView):
    model = Journal
    template_name = 'dashboards/transaction_list.html'
    paginate_by = 20
...
class AddTransactionView(LoginRequiredMixin, CreateView):
    ...
    template_name = 'dashboards/add_transaction.html'
...
class BalanceSheetView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        context = balance_sheet_view(request.user)
